...
mb.on('ready', function ready() {
  ...
  ipcMain.on('select-library', function() {
    dialog.showOpenDialog(
      {
        properties: ['openFile'],
        message: 'Locate Gifwit library file',
        filters: [{name: 'GifWit Library', extensions: ['gifwit']}],
      },
      fileNames => {
        ...
        if (libraryPath != undefined) {
          fs.readFile(libraryPath, 'utf-8', (err, data) => {
            ...
            var shouldOverwrite = dialog.showMessageBox({
              ...
              buttons: ['Yes', 'No'],
              message:
                'This will overwrite your existng library, are you sure?',
            });
  ...
  ipcMain.on('remove-from-library', function(data, returnVal) {
    var canDelete = dialog.showMessageBox({
      type: 'question',
      title: 'Are you sure?',
      buttons: ['Yes', 'No'],
      message: "Are you sure you wan't to delete this image?",
    });
